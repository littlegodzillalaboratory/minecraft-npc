...
class BreedAnimalsSkill extends BaseSkill {
  ...
  async do(opts) {
    ...
    if (animals.length < 2) {
      return this.fail(`I cannot find two ${opts.entityName} to breed nearby`);
    }
    ...
    if (!food) {
      return this.fail(`I have no food to breed the ${opts.entityName} with`);
    }

    await this.bot.equip(food, "hand");
    await this.bot.useOn(animals[0]);
    await this.bot.useOn(animals[1]);
    bag.logStepItemSuccess(`Fed two animals for ${opts.entityName} breeding`);
  }
}
