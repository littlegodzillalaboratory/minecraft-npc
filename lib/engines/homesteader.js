  ...
  _selectDecision(state) {
    ...
    if (threat && state.health <= this.config.fleeHealthThreshold) {
      bag.logStepItemSuccess(
        `Threat detected while health is low: ${threat.name} (${threat.distance} blocks away)`,
      );
      return {
    ...
    if (threat) {
      bag.logStepItemSuccess(
        `Threat detected: ${threat.name} (${threat.distance} blocks away)`,
      );
      return {
        goal: `defend against ${threat.name}`,
        action: new AttackNearestMobAction(this.npc),
        opts: {
          message: `attack nearest ${threat.name}`,
          messageElems: [undefined, threat.name],
          source: "auto-mode",
        },
      };
    }
