...
import CountEntitiesSkill from "./skills/count-entities.js";
import CountInventoryItemSkill from "./skills/count-inventory-item.js";
import FindNearbyThreatsSkill from "./skills/find-nearby-threats.js";
import FindNearestEntitySkill from "./skills/find-nearest-entity.js";
import FindNearestPlayerSkill from "./skills/find-nearest-player.js";
import GetEquippedArmorSkill from "./skills/get-equipped-armor.js";
import GetInventorySummarySkill from "./skills/get-inventory-summary.js";
import GetPlayerDistanceSkill from "./skills/get-player-distance.js";
import GetPlayerPositionSkill from "./skills/get-player-position.js";
import ListVisiblePlayersSkill from "./skills/list-visible-players.js";
import FindNearestAllowedAnimalSkill from "./skills/find-nearest-allowed-animal.js";
import GetSurvivalStateSkill from "./skills/get-survival-state.js";
import AutoMode from "./auto-mode.js";

class Npc {
  constructor(bot, register, opts) {
    this.bot = bot;
    // Actions own player-facing messages for all skills managed by this NPC.
    this.bot.deferSkillMessages = true;
    this.register = register;
    this.opts = opts;
    this.autoMode = new AutoMode(this, opts);
  }
  ...
  getPlayerPosition(player) {
    return new GetPlayerPositionSkill(this.bot).do({ player: player });
  }

  /** Count inventory items without exposing Mineflayer inventory structures. */
  countInventoryItem(itemName) {
    return new CountInventoryItemSkill(this.bot).do({ itemName: itemName });
  }

  /** Return display-ready inventory entries. */
  getInventorySummary() {
    return new GetInventorySummarySkill(this.bot).do({});
  }

  /** Return equipped armor item names. */
  getEquippedArmor() {
    return new GetEquippedArmorSkill(this.bot).do({});
  }

  /** Return visible player names other than the NPC. */
  listVisiblePlayers() {
    return new ListVisiblePlayersSkill(this.bot).do({});
  }

  /** Return the nearest other player entity. */
  findNearestPlayer() {
    return new FindNearestPlayerSkill(this.bot).do({});
  }

  /** Return the nearest entity matching a name. */
  findNearestEntity(entityName) {
    return new FindNearestEntitySkill(this.bot).do({ entityName: entityName });
  }

  /** Count loaded entities matching a name or category. */
  countEntities(entityName, maxDistance = 32) {
    return new CountEntitiesSkill(this.bot).do({
      entityName: entityName,
      maxDistance: maxDistance,
    });
  }

  /** Return hostile mob names within a maximum distance. */
  findNearbyThreats(maxDistance) {
    return new FindNearbyThreatsSkill(this.bot).do({
      maxDistance: maxDistance,
    });
  }

  /** Return the distance to a visible player. */
  getPlayerDistance(player) {
    return new GetPlayerDistanceSkill(this.bot).do({ player: player });
  }

  /** Enable auto-mode using a named autonomous policy engine. */
  enableAutoMode(engineName) {
    return this.autoMode.enable(engineName);
  }

  /** Disable autonomous decisions and cancel their current activity. */
  disableAutoMode() {
    return this.autoMode.disable();
  }

  /** Return the current auto-mode lifecycle state. */
  getAutoModeStatus() {
    return this.autoMode.getStatus();
  }

  /** Return the observations consumed by auto-mode survival policy. */
  getSurvivalState(threatRadius) {
    return new GetSurvivalStateSkill(this.bot).do({
      threatRadius: threatRadius,
    });
  }

  /** Find the nearest adult animal permitted by autonomous hunting policy. */
  findNearestAllowedAnimal(allowedAnimals, maximumDistance) {
    return new FindNearestAllowedAnimalSkill(this.bot).do({
  ...
  async guardLocation(posX, posY, posZ) {
    await this.moveToLocation(posX, posY, posZ);
  ...
  moveToObject(objectName) {
    ...
    const successMessage = `${this.bot.username} found object: ${objectName}`;
    const failureMessage = `Unable to find object: ${objectName}`;

    return this._doQuerySkill(
      skill,
      opts,
      validation,
      successMessage,
  ...
  wander() {
    ...
    const opts = {};
    const validation = () => true;
    const successMessage = `${this.bot.username} selected a wander destination`;
    const failureMessage = `Unable to select a wander destination`;
  ...
  huntFood(allowedAnimals, maximumDistance) {
    ...
    const validation = () =>
      v.isValidStringArray(allowedAnimals) &&
      v.isValidPositiveInteger(maximumDistance);
  ...
  flee() {
    ...
    const opts = {};
    const validation = () => true;
    const successMessage = `${this.bot.username} selected a flee destination`;
    const failureMessage = `Unable to select a flee destination`;
  ...
  collectItems() {
    ...
    const opts = {};
    const validation = () => true;
    const successMessage = `${this.bot.username} found a dropped item`;
    const failureMessage = `Unable to find dropped items`;
  ...
  async _doSkill(skill, opts, validation, successMessage, failureMessage) {
    if (validation()) {
      try {
        const result = await skill.do(opts);
        bag.logStepItemSuccess(successMessage);
        return result?.message
          ? { ...result, status: result.status || "success" }
          : "success";
      } catch (err) {
        bag.logStepItemError(`${failureMessage}: ${err.message}`);
        return {
          status: "failed",
          message: err.message,
        };
  ...
  /**
   * Execute a read-only domain skill and retain the value for action-level
   * orchestration. Query failures are returned instead of speaking directly.
   */
  async _doQuerySkill(skill, opts, validation, successMessage, failureMessage) {
    if (!validation()) {
      bag.logStepItemError(failureMessage);
    ...
    try {
      const value = await skill.do(opts);
      bag.logStepItemSuccess(successMessage);
