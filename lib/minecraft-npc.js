...
class MinecraftNpc {
  ...
  async start(cb) {
    ...
    _bot.chatgpt.setConfig(_opts.chatGptApiKey, { model: _opts.chatGptModel, instructions: _opts.chatGptInstructions });
    ...
    _bot.once('spawn', () => {
      ...
      mineflayerViewer.mineflayer(_bot, { port: _opts.viewerPort, firstPerson: false });
      ...
      _bot.on('chat', (username, message) => {
        ...
        if (username === _bot.username) {
          ...
          if (message.startsWith(_bot.username)) {
            ...
            const sanitisedMessage = message.trimStart(_bot.username).trimStart();
