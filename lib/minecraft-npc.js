...
import MoveInitLocationAction from './actions/move-init-location.js';
...
class MinecraftNpc {
  ...
  async start(cb) {
    ...
    const moveInitLocationAction = new MoveInitLocationAction(this.register, bot);
    const respondToMessageAction = new RespondToMessageAction(this.register, bot);
    const sayInitMessageAction = new SayInitMessageAction(this.register, bot);
      ...
      bot.on('chat', (username, message) => {
        ...
        if (username === bot.username) {
          ...
          if (message.startsWith(bot.username)) {
            ...
            const sanitisedMessage = message.trimStart(bot.username).trimStart();
          ...
          // (assumes bot is addressed if message starts with the bot username)
          } else {
            return;
          }
